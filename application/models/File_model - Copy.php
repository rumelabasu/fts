...
class File_model extends CI_Model
{
    ...
    public function registered_file_count() {
        return $this->db->count_all("fts_file_note");
    }
    ...
    public function fetch_file_data($limit, $start) {
       ...
       return $data_value;
		...
		{
            $file_reg_date=date('Y-m-d',strtotime($file_reg_date));
 ...
 public function count_file_date($file_reg_date)
    {
       ...
       return $data_value;
        ...
        $this->db->select('r.user_id,r.file_id,r.file_ref_sl_no,r.subject_id,r.file_name,r.description,m.received_date_time,m.from_desig_id,m.reciver_user_id,m.file_status');
        $this->db->from('fts_file_registration r');
        $this->db->join('fts_file_movement m', 'r.file_id=m.file_id','left');
    ...
    //file history
        ...
        $reciver_section=array();
             foreach ($result as $row) 
             {
                ...
                $sender_section[$row['trail_id']]=$this->section_name($sender[0]['sec_id']);
                ...
                if(isset($receiver_id)&& $receiver_id !="" )
                {
                 ...
                 $reciver_section[$row['trail_id']]=$this->section_name($receiver[0]['sec_id']);
                }
...
// fetch_file_inbox
    ...
    public function fetch_reg_file($limit, $start) {
        $user_id=$this->session->userdata('user_id');
        $this->db->select('r.file_id,r.file_ref_sl_no,r.sec_id,r.subject_id,r.file_reg_date,r.file_name,r.file_move_status,s.subject_name,se.sec_name');
        $this->db->from('fts_file_registration r');
        $this->db->join('fts_subject s', 'r.subject_id =s.subject_id','left');
   ...
   public function designation($condition) 
    ...
    public function file_dispatch($file_id) {
        $this->db->select('r.file_id,r.file_name,r.file_ref_sl_no,r.sec_id,r.subject_id,r.file_reg_date,m.file_id m_file_id,m.from_desig_id,m.received_date_time,m.file_status,m.dispatch_key,s.subject_name,se.sec_name');
        $this->db->from('fts_file_registration r');
        $this->db->join('fts_file_movement m', 'r.file_id =m.file_id','left');
        $this->db->join('fts_subject s', 'r.subject_id =s.subject_id','left');
        $this->db->join('fts_section se', 'r.sec_id =se.sec_id','left');
