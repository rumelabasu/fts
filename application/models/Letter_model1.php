...
class Letter_model extends CI_Model
{
    ...
    public function fetch_file_data($limit, $start) {
       ...
       //echo $this->db->last_query();
        ...
        {
        $reg_dt=date('Y-m-d',strtotime($reg_dt));
        //$this->db->select('*');
        $this->db->select('r.letter_id,r.memo_no,r.file_id,r.reg_dt,r.user_id,m.letter_id,m.move_id');
 ...
 public function count_letter_date($reg_dt)
    {
       ...
       return $data_value;
        ...
        public function track_letter_bysubject($description,$limit,$start)
        {
            ...
            $this->db->select('r.letter_id,r.subject,r.memo_no,r.file_id,r.reg_dt,r.sending_authority,r.user_id,m.letter_id,m.move_id,m.receiver_id');
            $this->db->from('fts_letter_record r');
            $this->db->join('fts_letter_movement m', 'r.letter_id=m.letter_id','left');
...
// fetch_letter_inbox
    public function fetch_letter_inbox($limit, $start) {
        $user_id=$this->session->userdata('user_id');
        $this->db->select('r.letter_id,r.memo_no,r.reg_dt,r.issue_dt,r.subject,r.letter_name,m.receiver_id,m.dispatch_dt_time,m.sender_id,m.sender_desig_id,a.authority_name,ac.action_details,ac.deadline_dt,ac.action_id');
        $this->db->from('fts_letter_record r');
...
//count total row using description
        ...
        {
             $text=$text!=""?$text:'NULL';
            $query = $this->db->query("SELECT * FROM fts_letter_record where content like '%$text%' ");
            return $query->num_rows(); 
        }
 ...
 //track file bysending authority
       public function track_letter_bysending_authority($authority,$limit,$start)
        { 
            ...
            $this->db->select('r.letter_id,r.memo_no,r.file_id,r.reg_dt,r.sending_authority,r.user_id,m.letter_id,m.move_id,m.receiver_id');
            $this->db->from('fts_letter_record r');
            $this->db->join('fts_letter_movement m', 'r.letter_id=m.letter_id','left');
