...
//count total row using category
        public function count_letter_bycategory($category)
        {
            $query = $this->db->query("SELECT * FROM fts_letter_record where register_id='$category'");
            return $query->num_rows(); 
        }
        //track letter bysubject
        public function track_letter_bycategory($category,$limit,$start)
        {
            ...
            $this->db->join('fts_letter_movement m', 'r.letter_id=m.letter_id','left');
             $this->db->join('fts_letter_register re', 'r.register_id=re.register_id','left');
            $this->db->join('fts_authority au', 'r.sending_authority=au.authority_id','left');
            $this->db->like('r.register_id',$category);
            $this->db->limit($limit, $start);
            $query = $this->db->get();
            $result=$query->result_array();
